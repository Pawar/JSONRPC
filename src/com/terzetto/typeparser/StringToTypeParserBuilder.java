...
public final class StringToTypeParserBuilder {
	...
	static {
		...
		for (TypeParser<?> typeParser : DefaultTypeParsers.list()) {
			...
			if (wrapperToPrimitiveMapper.containsKey(type)) {
				...
				defaultTypeParsers.put(wrapperToPrimitiveMapper.get(type),
						typeParser);
			}
		}
	}
	...
	public StringToTypeParserBuilder registerTypeParser(TypeParser<?> typeParser) {
		if (typeParser == null) {
			throw new NullPointerException(
					Util.nullArgumentErrorMsg("typeParser"));
		}
		typeParsers.put(Util.extractTypeParameter(typeParser), typeParser);
		return this;
	}

	public <T> StringToTypeParserBuilder registerTypeParser(Class<T> type,
			TypeParser<? extends T> typeParser) {
		if (typeParser == null) {
			throw new NullPointerException(
					Util.nullArgumentErrorMsg("typeParser"));
		}
	...
	public StringToTypeParser build() {
		return new StringToTypeParser(new HashMap<Class<?>, TypeParser<?>>(
				typeParsers));
	}
}