...
public final class StringToTypeParser {
	private static final StringToTypeParser defaultTypeParser = newBuilder()
			.build();
	...
	public <T> T parseType(String value, Class<T> type) {
		...
		if (typeParsers.containsKey(type)) {
			...
			message = String.format(message, type.getSimpleName(),
					type.getSimpleName());
	...
	private Object callTypeParser(String value, Class<?> type) {
		try {
			...
			String message = canNotParseErrorMsg(value, type,
					numberFormatErrorMsg(e));
			throw new IllegalArgumentException(message, e);
		} catch (RuntimeException e) {
			throw new IllegalArgumentException(canNotParseErrorMsg(value, type,
					e.getMessage()), e);
		}
	}
	...
	private static String canNotParseErrorMsg(String value, Class<?> type,
			String message) {
		return String.format("Can not parse \"%s\" to type '%s' due to: %s",
				value, type.getName(), message);
	}

	private static Object callFactoryMethodIfExisting(String methodName,
		...
		try {
			...
			throw new IllegalArgumentException(makeErrorMsg(methodName, value,
					type), e.getCause());
		} catch (Throwable t) {
			throw new IllegalArgumentException(makeErrorMsg(methodName, value,
					type), t);
	...
	private static String makeErrorMsg(String methodName, String value,
			Class<?> type) {
		String methodSignature = String.format("%s.%s('%s')", type.getName(),
				methodName, value);
		String message = " Exception thrown in static factory method '%s'. See underlying "
				+ "exception for additional information.";
		return canNotParseErrorMsg(value, type,
				String.format(message, methodSignature));
	}
}